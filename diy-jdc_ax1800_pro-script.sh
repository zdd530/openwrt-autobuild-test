...
fix_mk_def_depends() {
    ...
    if [ -f $OPENWRT_PATH/target/linux/qualcommax/Makefile ]; then
        sed -i 's/wpad-basic-mbedtls/wpad-openssl/g' $OPENWRT_PATH/target/linux/qualcommax/Makefile
    fi
}

add_wifi_default_set() {
    local uci_dir="$OPENWRT_PATH/package/base-files/files/etc/uci-defaults"
    local ipq_uci_dir="$OPENWRT_PATH/target/linux/qualcommax/ipq60xx/base-files/etc/uci-defaults"
...
remove_something_nss_kmod() {
    local ipq_target_path="$OPENWRT_PATH/target/linux/qualcommax/ipq60xx/target.mk"
...
remove_affinity_script() {
    local affinity_script_path="$OPENWRT_PATH/target/linux/qualcommax/ipq60xx/base-files/etc/init.d/set-irq-affinity"
...
fix_build_for_openssl() {
    local makefile="$OPENWRT_PATH/package/libs/openssl/Makefile"
    if [[ -f "$makefile" ]]; then
...
chanage_cpuusage() {
    ...
    local imm_script2="$OPENWRT_PATH/target/linux/qualcommax/ipq60xx/base-files/sbin/cpuusage"
